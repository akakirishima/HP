// Work page
import { useMemo, useState } from 'react';
import { Link } from 'react-router-dom';
import { useLanguage } from "../contexts/LanguageContext";

type Category =
  | 'education'
  | 'research'
  | 'study_abroad'
  | 'clubs'
  | 'part_time'
  | 'awards_other';

type ClubGroup = 'dotcube' | 'wanderfogel';

type Experience = {
  category: Category;
  clubGroup?: ClubGroup;
  date_ja: string;
  date_en: string;
  date_ko: string;
  title_ja: string;
  title_en: string;
  title_ko: string;
  desc_ja: string;
  desc_en: string;
  desc_ko: string;
  href?: string;
};

const experiences: Experience[] = [
  {
    category: 'education',
    date_ja: '2028-03（予定）',
    date_en: 'Mar 2028 (Planned)',
    date_ko: '2028-03(예정)',
    title_ja: '修士修了（28卒予定）',
    title_en: 'Master’s Degree Completion (Planned)',
    title_ko: '석사 과정 수료(28졸 예정)',
    desc_ja: '修士課程修了予定',
    desc_en: 'Planned master’s completion',
    desc_ko: '석사 과정 수료 예정',
    href: '/blog/education-masters-completion-plan',
  },
  {
    category: 'research',
    date_ja: '2026-04 〜 2028-03（予定）',
    date_en: 'Apr 2026 – Mar 2028 (Planned)',
    date_ko: '2026-04 ~ 2028-03(예정)',
    title_ja: '修士課程（Thi Thi Zin 研究室）',
    title_en: 'Master’s Program (Thi Thi Zin Lab)',
    title_ko: '석사 과정(Thi Thi Zin 연구실)',
    desc_ja: 'Thi Thi Zin 研究室 所属予定',
    desc_en: 'Planned lab membership',
    desc_ko: 'Thi Thi Zin 연구실 소속 예정',
    href: '/blog/research-masters-plan',
  },
  {
    category: 'awards_other',
    date_ja: '2026-03-29',
    date_en: 'Mar 29, 2026',
    date_ko: '2026-03-29',
    title_ja: 'GDGoC Japan Hackathon 2026 決勝 ACMEE賞',
    title_en: 'GDGoC Japan Hackathon 2026 Final — ACMEE Award',
    title_ko: 'GDGoC Japan Hackathon 2026 결승 ACMEE상',
    desc_ja: '本戦・決勝でACMEE賞受賞',
    desc_en: 'Received the ACMEE Award at the final round',
    desc_ko: '본선·결승에서 ACMEE상 수상',
  },
  {
    category: 'awards_other',
    date_ja: '2026-03-22',
    date_en: 'Mar 22, 2026',
    date_ko: '2026-03-22',
    title_ja: 'GDGoC Japan Hackathon 2026 福岡会場 最優秀賞',
    title_en: 'GDGoC Japan Hackathon 2026 Fukuoka — Grand Prize',
    title_ko: 'GDGoC Japan Hackathon 2026 후쿠오카 회장 최우수상',
    desc_ja: '福岡会場で優勝・決勝進出',
    desc_en: 'Won at the Fukuoka venue and advanced to the final',
    desc_ko: '후쿠오카 회장에서 우승하고 결승 진출',
  },
  {
    category: 'awards_other',
    date_ja: '2026-02',
    date_en: 'Feb 2026',
    date_ko: '2026-02',
    title_ja: 'AI Agent Challenge 2026 参加',
    title_en: 'Participated in AI Agent Challenge 2026',
    title_ko: 'AI Agent Challenge 2026 참가',
    desc_ja: '大会参加',
    desc_en: 'Competition participation',
    desc_ko: '대회 참가',
    href: '/blog/ai-agent-challenge-2026',
  },
  {
    category: 'research',
    date_ja: '2025-04 〜 2026-03（学部4年）',
    date_en: 'Apr 2025 – Mar 2026',
    date_ko: '2025-04 ~ 2026-03(학부 4학년)',
    title_ja: 'Thi Thi Zin 研究室 配属',
    title_en: 'Thi Thi Zin Lab',
    title_ko: 'Thi Thi Zin 연구실 배속',
    desc_ja: '学部4年研究室配属',
    desc_en: 'Lab assignment in senior year',
    desc_ko: '학부 4학년 연구실 배속',
    href: '/blog/research-thi-thi-zin',
  },
  {
    category: 'part_time',
    date_ja: '2025-04 〜 2026-03（大学4年）',
    date_en: 'Apr 2025 – Mar 2026',
    date_ko: '2025-04 ~ 2026-03(대학 4학년)',
    title_ja: 'アルバイト：ラヴィアンローズ（宮崎パフェバー）',
    title_en: 'Part-time: Lavie en Rose (Miyazaki Parfait Bar)',
    title_ko: '아르바이트: 라비앙 로즈(미야자키 파르페 바)',
    desc_ja: 'アルバイト',
    desc_en: 'Part-time job',
    desc_ko: '아르바이트',
    href: '/blog/part-time-lavie-en-rose',
  },
  {
    category: 'awards_other',
    date_ja: '2025-04',
    date_en: 'Apr 2025',
    date_ko: '2025-04',
    title_ja: '技育博 予選通過',
    title_en: 'Gikyu Expo Qualifier',
    title_ko: '기큐 박람회 예선 통과',
    desc_ja: 'サポーターズ主催',
    desc_en: 'Supporters Inc. event',
    desc_ko: 'Supporters 주최',
    href: '/blog/award-gikyu-expo-qualifier',
  },
  {
    category: 'awards_other',
    date_ja: '2025-02',
    date_en: 'Feb 2025',
    date_ko: '2025-02',
    title_ja: '技育CAMPハッカソン vol.23 参加',
    title_en: 'Gikyu CAMP Hackathon vol.23',
    title_ko: '기큐 CAMP 해커톤 vol.23 참가',
    desc_ja: 'サポーターズ主催',
    desc_en: 'Supporters Inc. event',
    desc_ko: 'Supporters 주최',
    href: '/blog/award-gikyu-camp-vol23',
  },
  {
    category: 'part_time',
    date_ja: '2024-10 〜 2026-03（大学3年〜4年）',
    date_en: 'Oct 2024 – Mar 2026',
    date_ko: '2024-10 ~ 2026-03(대학 3~4학년)',
    title_ja: 'アルバイト：WOW’D PLUS',
    title_en: 'Part-time: WOW’D PLUS',
    title_ko: '아르바이트: WOW’D PLUS',
    desc_ja: 'アルバイト',
    desc_en: 'Part-time job',
    desc_ko: '아르바이트',
    href: '/blog/part-time-wowd-plus',
  },
  {
    category: 'clubs',
    clubGroup: 'wanderfogel',
    date_ja: '2024-11-09 〜 2024-11-10',
    date_en: 'Nov 9–10, 2024',
    date_ko: '2024-11-09 ~ 2024-11-10',
    title_ja: '由布岳 班PW',
    title_en: 'Yufu-dake Team PW',
    title_ko: '유후다케 팀 PW',
    desc_ja: 'ワンダーフォーゲル部 山行',
    desc_en: 'Wanderfogel club trip',
    desc_ko: 'Wanderfogel 동아리 산행',
    href: '/blog/mountain-yufu',
  },
  {
    category: 'awards_other',
    date_ja: '2024-09-24 〜 2024-09-25',
    date_en: 'Sep 24–25, 2024',
    date_ko: '2024-09-24 ~ 2024-09-25',
    title_ja: '第二回 宮崎ハッカソン 最優秀賞',
    title_en: '2nd Miyazaki Hackathon — Grand Prize',
    title_ko: '제2회 미야자키 해커톤 최우수상',
    desc_ja: '主催：カラビナテクノロジー',
    desc_en: 'Hosted by Carabiner Technology',
    desc_ko: '주최: Carabiner Technology',
    href: '/blog/award-miyazaki-hackathon-2nd',
  },
  {
    category: 'study_abroad',
    date_ja: '2024-08-21 〜 2024-08-28',
    date_en: 'Aug 21–28, 2024',
    date_ko: '2024-08-21 ~ 2024-08-28',
    title_ja: '韓国・順天大学校 STEP3 短期派遣留学',
    title_en: 'Short-term Study Abroad: Sunchon National University (STEP3)',
    title_ko: '한국 순천대학교 STEP3 단기 파견 유학',
    desc_ja: '8日間',
    desc_en: '8 days',
    desc_ko: '8일',
    href: '/blog/step3-suncheon',
  },
  {
    category: 'clubs',
    clubGroup: 'wanderfogel',
    date_ja: '2024-05-18 〜 2024-05-19',
    date_en: 'May 18–19, 2024',
    date_ko: '2024-05-18 ~ 2024-05-19',
    title_ja: '開聞岳 登山',
    title_en: 'Kaimon-dake Hike',
    title_ko: '가이몬다케 등산',
    desc_ja: 'ワンダーフォーゲル部 山行',
    desc_en: 'Wanderfogel club trip',
    desc_ko: 'Wanderfogel 동아리 산행',
    href: '/blog/mountain-kaimon',
  },
  {
    category: 'study_abroad',
    date_ja: '2024-03-01 / 2024-03-08 / 2024-03-15',
    date_en: 'Mar 1 / 8 / 15, 2024',
    date_ko: '2024-03-01 / 2024-03-08 / 2024-03-15',
    title_ja: 'ALI 6920（Academic Research）実施日',
    title_en: 'ALI 6920 (Academic Research) Sessions',
    title_ko: 'ALI 6920(Academic Research) 진행일',
    desc_ja: 'IUP（ALI）オンライン留学',
    desc_en: 'IUP (ALI) online program',
    desc_ko: 'IUP(ALI) 온라인 유학',
    href: '/blog/ali-6920',
  },
  {
    category: 'awards_other',
    date_ja: '2024-02-29',
    date_en: 'Feb 29, 2024',
    date_ko: '2024-02-29',
    title_ja: '宮崎大学データサイエンスコンペ 最優秀賞',
    title_en: 'Miyazaki Univ. Data Science Competition — Grand Prize',
    title_ko: '미야자키대학교 데이터사이언스 컴페티션 최우수상',
    desc_ja: '宮崎大学主催',
    desc_en: 'Hosted by Miyazaki University',
    desc_ko: '미야자키대학교 주최',
    href: '/blog/award-miyazaki-ds-competition',
  },
  {
    category: 'awards_other',
    date_ja: '2024-02-15 〜 2024-02-16',
    date_en: 'Feb 15–16, 2024',
    date_ko: '2024-02-15 ~ 2024-02-16',
    title_ja: '宮崎ハッカソン 最優秀賞',
    title_en: 'Miyazaki Hackathon — Grand Prize',
    title_ko: '미야자키 해커톤 최우수상',
    desc_ja: '主催：カラビナテクノロジー',
    desc_en: 'Hosted by Carabiner Technology',
    desc_ko: '주최: Carabiner Technology',
    href: '/blog/award-miyazaki-hackathon-1st',
  },
  {
    category: 'study_abroad',
    date_ja: '2024-02-12 〜 2024-03-29',
    date_en: 'Feb 12 – Mar 29, 2024',
    date_ko: '2024-02-12 ~ 2024-03-29',
    title_ja: 'IUP（ALI）オンライン留学',
    title_en: 'IUP (ALI) Online Study Abroad',
    title_ko: 'IUP(ALI) 온라인 유학',
    desc_ja: '米国 IUP / Virtual classes',
    desc_en: 'Indiana University of Pennsylvania / virtual classes',
    desc_ko: '미국 IUP / 온라인 수업',
    href: '/blog/iup-ali-online',
  },
  {
    category: 'clubs',
    clubGroup: 'wanderfogel',
    date_ja: '2023-08-17 〜 2023-08-20',
    date_en: 'Aug 17–20, 2023',
    date_ko: '2023-08-17 ~ 2023-08-20',
    title_ja: '立山（夏合宿）',
    title_en: 'Tateyama (Summer Camp)',
    title_ko: '다테야마(여름 합숙)',
    desc_ja: 'ワンダーフォーゲル部 山行',
    desc_en: 'Wanderfogel club trip',
    desc_ko: 'Wanderfogel 동아리 산행',
    href: '/blog/mountain-tateyama',
  },
  {
    category: 'clubs',
    clubGroup: 'wanderfogel',
    date_ja: '2023-05-27 〜 2023-05-28',
    date_en: 'May 27–28, 2023',
    date_ko: '2023-05-27 ~ 2023-05-28',
    title_ja: '釈迦岳（大分・福岡）山行',
    title_en: 'Shaka-dake (Oita/Fukuoka) Hike',
    title_ko: '샤카다케(오이타/후쿠오카) 산행',
    desc_ja: 'ワンダーフォーゲル部 山行',
    desc_en: 'Wanderfogel club trip',
    desc_ko: 'Wanderfogel 동아리 산행',
    href: '/blog/mountain-shaka',
  },
  {
    category: 'clubs',
    clubGroup: 'dotcube',
    date_ja: '2023-04 〜 2024-03（大学2年）',
    date_en: 'Apr 2023 – Mar 2024',
    date_ko: '2023-04 ~ 2024-03(대학 2학년)',
    title_ja: '学祭「宮大タワーバトル」制作・出展',
    title_en: 'University Festival: “Miyazaki Tower Battle”',
    title_ko: '학제 “미야자키 타워 배틀” 제작·출전',
    desc_ja: '来場200人',
    desc_en: '200 visitors',
    desc_ko: '방문 200명',
    href: '/blog/festival-miyazaki-tower-battle',
  },
  {
    category: 'clubs',
    clubGroup: 'wanderfogel',
    date_ja: '2023-03-27 〜 2023-03-31',
    date_en: 'Mar 27–31, 2023',
    date_ko: '2023-03-27 ~ 2023-03-31',
    title_ja: '屋久島・宮之浦岳（春合宿）',
    title_en: 'Yakushima / Miyanoura-dake (Spring Camp)',
    title_ko: '야쿠시마·미야노우라다케(봄 합숙)',
    desc_ja: 'ワンダーフォーゲル部 合宿',
    desc_en: 'Wanderfogel club camp',
    desc_ko: 'Wanderfogel 동아리 합숙',
    href: '/blog/mountain-yakushima-miyanoura',
  },
  {
    category: 'part_time',
    date_ja: '2023-03 〜 2024-09（大学2年〜3年）',
    date_en: 'Mar 2023 – Sep 2024',
    date_ko: '2023-03 ~ 2024-09(대학 2~3학년)',
    title_ja: 'アルバイト：ラウンドワン',
    title_en: 'Part-time: Round1',
    title_ko: '아르바이트: Round1',
    desc_ja: 'アルバイト',
    desc_en: 'Part-time job',
    desc_ko: '아르바이트',
    href: '/blog/part-time-round1',
  },
  {
    category: 'clubs',
    clubGroup: 'wanderfogel',
    date_ja: '2022-10-29 〜 2022-10-30',
    date_en: 'Oct 29–30, 2022',
    date_ko: '2022-10-29 ~ 2022-10-30',
    title_ja: '白髪岳 PW',
    title_en: 'Shiraga-dake PW',
    title_ko: '시라가다케 PW',
    desc_ja: 'ワンダーフォーゲル部 山行',
    desc_en: 'Wanderfogel club trip',
    desc_ko: 'Wanderfogel 동아리 산행',
    href: '/blog/mountain-shiraga',
  },
  {
    category: 'clubs',
    clubGroup: 'dotcube',
    date_ja: '2022-04 〜 2023-03（大学1年）',
    date_en: 'Apr 2022 – Mar 2023',
    date_ko: '2022-04 ~ 2023-03(대학 1학년)',
    title_ja: '学祭でKinectゲーム制作に挑戦',
    title_en: 'University Festival: Kinect Game Challenge',
    title_ko: '학제에서 Kinect 게임 제작 도전',
    desc_ja: '来場5人',
    desc_en: '5 visitors',
    desc_ko: '방문 5명',
    href: '/blog/festival-kinect',
  },
  {
    category: 'part_time',
    date_ja: '2022-04 〜 2023-03（大学1年）',
    date_en: 'Apr 2022 – Mar 2023',
    date_ko: '2022-04 ~ 2023-03(대학 1학년)',
    title_ja: 'アルバイト：smolt',
    title_en: 'Part-time: smolt',
    title_ko: '아르바이트: smolt',
    desc_ja: 'アルバイト',
    desc_en: 'Part-time job',
    desc_ko: '아르바이트',
    href: '/blog/part-time-smolt',
  },
  {
    category: 'clubs',
    clubGroup: 'dotcube',
    date_ja: '2022-04 〜（期間不明）',
    date_en: 'Apr 2022 – (Unknown)',
    date_ko: '2022-04 ~ (기간 미정)',
    title_ja: '学生団体 dotcube（執行部）',
    title_en: 'Student Org: dotcube (Executive)',
    title_ko: '학생단체 dotcube(집행부)',
    desc_ja: '執行部で部を牽引',
    desc_en: 'Executive team member',
    desc_ko: '집행부로서 조직을 이끌음',
    href: '/blog/club-dotcube',
  },
  {
    category: 'clubs',
    clubGroup: 'dotcube',
    date_ja: '（時期不明）',
    date_en: '(Unknown)',
    date_ko: '(시기 미정)',
    title_ja: 'dotcube AI活用HP作成講座 講師',
    title_en: 'dotcube AI Website Workshop Instructor',
    title_ko: 'dotcube AI 활용 홈페이지 제작 강좌 강사',
    desc_ja: '小学生向け講座',
    desc_en: 'Workshop for elementary students',
    desc_ko: '초등학생 대상 강좌',
    href: '/blog/club-dotcube-workshop',
  },
  {
    category: 'clubs',
    clubGroup: 'wanderfogel',
    date_ja: '2022-04 〜（期間不明）',
    date_en: 'Apr 2022 – (Unknown)',
    date_ko: '2022-04 ~ (기간 미정)',
    title_ja: '宮崎大学ワンダーフォーゲル部 所属',
    title_en: 'Miyazaki Univ. Wanderfogel Club',
    title_ko: '미야자키대학교 Wanderfogel부 소속',
    desc_ja: '山行・合宿に参加',
    desc_en: 'Trips and camps',
    desc_ko: '산행·합숙 참가',
    href: '/blog/club-wanderfogel',
  },
  {
    category: 'education',
    date_ja: '2022-04 〜 2026-03',
    date_en: 'Apr 2022 – Mar 2026',
    date_ko: '2022-04 ~ 2026-03',
    title_ja: '宮崎大学 工学部 情報通信工学プログラム 在籍',
    title_en: 'Miyazaki University (Engineering / ICT Program)',
    title_ko: '미야자키대학교 공학부 정보통신공학 프로그램 재학',
    desc_ja: '工学部 工学科',
    desc_en: 'Faculty of Engineering',
    desc_ko: '공학부 공학과',
    href: '/blog/education-miyazaki-university',
  },
  {
    category: 'education',
    date_ja: '2003-07-02',
    date_en: 'Jul 2, 2003',
    date_ko: '2003-07-02',
    title_ja: '出生',
    title_en: 'Born',
    title_ko: '출생',
    desc_ja: '誕生',
    desc_en: 'Birth',
    desc_ko: '탄생',
    href: '/blog/education-birth',
  },
];

export default function WorkPage() {
  const { t, language } = useLanguage();
  const categories: { id: Category; label_ja: string; label_en: string; label_ko: string; label_zh: string }[] = [
    { id: 'education', label_ja: '学歴', label_en: 'Education', label_ko: '학력', label_zh: '学历' },
    { id: 'research', label_ja: '研究', label_en: 'Research', label_ko: '연구', label_zh: '研究' },
    { id: 'study_abroad', label_ja: '留学', label_en: 'Study Abroad', label_ko: '유학', label_zh: '留学' },
    { id: 'clubs', label_ja: '部活', label_en: 'Clubs', label_ko: '동아리', label_zh: '社团' },
    { id: 'part_time', label_ja: 'アルバイト', label_en: 'Part-time', label_ko: '아르바이트', label_zh: '兼职' },
    { id: 'awards_other', label_ja: '受賞・その他', label_en: 'Awards & Other', label_ko: '수상·기타', label_zh: '获奖与其他' },
  ];

  const defaultSelected: Category[] = ['education', 'awards_other', 'research', 'study_abroad'];
  const [selectedCategories, setSelectedCategories] = useState<Category[]>(defaultSelected);
  const [selectedClubGroups, setSelectedClubGroups] = useState<ClubGroup[]>([
    'dotcube',
    'wanderfogel',
  ]);
  const [presetMode, setPresetMode] = useState<'core' | 'all' | null>('core');

  const filteredExperiences = useMemo(() => {
    if (selectedCategories.length === 0) return [];
    return experiences.filter((exp) => {
      if (!selectedCategories.includes(exp.category)) return false;
      if (exp.category !== 'clubs') return true;
      if (!exp.clubGroup) return true;
      return selectedClubGroups.includes(exp.clubGroup);
    });
  }, [selectedCategories, selectedClubGroups]);

  return (
    <main className="section page page--medium">
      <h1 className="section-title animate-fade-in-up">
        {t('work_title')}
      </h1>
      <p className="animate-fade-in-up animate-delay-1" style={{ color: '#666', marginBottom: '3rem', marginTop: '1rem' }}>
        {t('work_desc')}
      </p>
      {language === 'zh' && (
        <p style={{ color: '#666', marginTop: '-2rem', marginBottom: '2rem' }}>
          {t('content_notice_en_fallback')}
        </p>
      )}

      <section className="category-chips">
        <div className="chip-controls">
          <button
            type="button"
            className={`chip chip--ghost ${presetMode === 'core' ? 'chip--active' : ''}`}
            onClick={() => {
              if (presetMode === 'core') {
                setSelectedCategories([]);
                setPresetMode(null);
                return;
              }
              setSelectedCategories(defaultSelected);
              setSelectedClubGroups(['dotcube', 'wanderfogel']);
              setPresetMode('core');
            }}
          >
            {language === 'ja' ? '主要のみ' : language === 'ko' ? '핵심만' : language === 'zh' ? '仅核心' : 'Core Only'}
          </button>
          <button
            type="button"
            className={`chip chip--ghost ${presetMode === 'all' ? 'chip--active' : ''}`}
            onClick={() => {
              if (presetMode === 'all') {
                setSelectedCategories([]);
                setPresetMode(null);
                return;
              }
              setSelectedCategories(categories.map((c) => c.id));
              setSelectedClubGroups(['dotcube', 'wanderfogel']);
              setPresetMode('all');
            }}
          >
            {language === 'ja' ? '全て' : language === 'ko' ? '전체' : language === 'zh' ? '全部' : 'All'}
          </button>
        </div>
        {categories.map((cat) => {
          const isActive = selectedCategories.includes(cat.id);
          const isClubs = cat.id === 'clubs';

          return (
            <div key={cat.id} className="chip-group">
              <button
                type="button"
                className={`chip ${isActive ? 'chip--active' : ''}`}
              onClick={() => {
                setSelectedCategories((prev) => {
                  if (prev.includes(cat.id)) {
                    return prev.filter((c) => c !== cat.id);
                  }
                  return [...prev, cat.id];
                });
                setPresetMode(null);
              }}
            >
                {language === 'ja' ? cat.label_ja : language === 'ko' ? cat.label_ko : language === 'zh' ? cat.label_zh : cat.label_en}
              </button>

              {isClubs && isActive && (
                <div className="chip-group__children">
                    {[
                    { id: 'dotcube' as const, label_ja: 'Dotcube', label_en: 'Dotcube', label_ko: 'Dotcube', label_zh: 'Dotcube' },
                    { id: 'wanderfogel' as const, label_ja: 'ワンダーフォーゲル', label_en: 'Wanderfogel', label_ko: 'Wanderfogel', label_zh: 'Wanderfogel' },
                  ].map((group) => {
                    const isGroupActive = selectedClubGroups.includes(group.id);
                    return (
                      <button
                        key={group.id}
                        type="button"
                        className={`chip ${isGroupActive ? 'chip--active' : ''}`}
                          onClick={() => {
                            setSelectedClubGroups((prev) => {
                              if (prev.includes(group.id)) {
                                return prev.filter((c) => c !== group.id);
                              }
                              return [...prev, group.id];
                            });
                            setPresetMode(null);
                          }}
                        >
                        {language === 'ja' ? group.label_ja : language === 'ko' ? group.label_ko : language === 'zh' ? group.label_zh : group.label_en}
                      </button>
                    );
                  })}
                </div>
              )}
            </div>
          );
        })}
      </section>

      <div className="timeline">
        {filteredExperiences.length === 0 ? (
          <div className="timeline-empty">
            {language === 'ja'
              ? '表示するカテゴリが選択されていません。'
              : language === 'ko'
              ? '선택된 카테고리가 없습니다.'
              : language === 'zh'
              ? '未选择要显示的分类。'
              : 'No categories selected.'}
          </div>
        ) : (
          filteredExperiences.map((exp, i) => (
            <div
              key={i}
              className={`timeline-item ${exp.href ? 'timeline-item--link' : ''}`}
              style={{ animationDelay: `${0.15 * i}s` }}
            >
              {exp.href ? (
                exp.href.startsWith('/') ? (
                  <Link to={exp.href} className="timeline-link">
                    <div className="timeline-date">
                      {language === 'ja' ? exp.date_ja : language === 'ko' ? exp.date_ko : exp.date_en}
                    </div>
                    <div className="timeline-title">
                      {language === 'ja' ? exp.title_ja : language === 'ko' ? exp.title_ko : exp.title_en}
                    </div>
                    <div className="timeline-desc">
                      {language === 'ja' ? exp.desc_ja : language === 'ko' ? exp.desc_ko : exp.desc_en}
                    </div>
                  </Link>
                ) : (
                  <a href={exp.href} className="timeline-link" target="_blank" rel="noopener noreferrer">
                    <div className="timeline-date">
                      {language === 'ja' ? exp.date_ja : language === 'ko' ? exp.date_ko : exp.date_en}
                    </div>
                    <div className="timeline-title">
                      {language === 'ja' ? exp.title_ja : language === 'ko' ? exp.title_ko : exp.title_en}
                    </div>
                    <div className="timeline-desc">
                      {language === 'ja' ? exp.desc_ja : language === 'ko' ? exp.desc_ko : exp.desc_en}
                    </div>
                  </a>
                )
              ) : (
                <>
                  <div className="timeline-date">
                    {language === 'ja' ? exp.date_ja : language === 'ko' ? exp.date_ko : exp.date_en}
                  </div>
                  <div className="timeline-title">
                    {language === 'ja' ? exp.title_ja : language === 'ko' ? exp.title_ko : exp.title_en}
                  </div>
                  <div className="timeline-desc">
                    {language === 'ja' ? exp.desc_ja : language === 'ko' ? exp.desc_ko : exp.desc_en}
                  </div>
                </>
              )}
            </div>
          ))
        )}
      </div>

    </main>
  );
}
